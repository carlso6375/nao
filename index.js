...
const vcard = 'BEGIN:VCARD\n' // metadata of the contact card
            + 'VERSION:3.0\n' 
            + 'FN:CARLOS O GOXTOSO AMOR DA SUA VIDA😳❤\n' // full name
            + 'ORG:Owner Bot;\n' // the organization of the contact
            + 'TEL;type=CELL;type=VOICE;waid=5588981321692:+55 (88) 8132-1692\n' // ID do WhatsApp + número de telefone
            + 'END:VCARD'
/******END OF VCARD INPUT******/
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Vai com deus e a pomba do divindo @${num.split('@')[0]}👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
				...
				case 'meme':
					...
					break
              case 'melhor do mundo':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQszf78yHGIGm32BaQQfWFeEbuwV0CDw7tHWA&usqp=CAU`)
		...
		if (isLimit(sender)) return reply(limitend(pushname2))
				...
				case 'alay':
					...
					client.groupLeave (from) 
                     ...
                     setTimeout( () => {
					client.updatePresence(from, Presence.composing) 
					client.sendMessage(from, 'vai com deus😪👋', text) // ur cods
					}, 0)
                     break
